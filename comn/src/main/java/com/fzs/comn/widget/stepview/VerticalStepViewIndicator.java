...
public class VerticalStepViewIndicator extends View {
	...
	private int mUnCompletedLineColor = getContext().getResources().getColor(
			R.color.green);// 定义默认未完成线的颜色 definition
										// mUnCompletedLineColor
	private int mCompletedLineColor = getResources().getColor(R.color.text_hint_color);// 定义默认完成线的颜色 definition
													// mCompletedLineColor
	...
	private void init() {
		...
		mCompleteIcon = getContext().getResources().getDrawable(R.mipmap.complted);// 已经完成的icon
		mAttentionIcon = getContext().getResources().getDrawable(R.mipmap.attention);// 正在进行的icon
		mDefaultIcon = getContext().getResources().getDrawable(R.mipmap.default_icon);// 未完成的icon
	...
	protected void onSizeChanged() {
		...
		for (int i = 0; i < mStepNum; i++) {
			...
			if (mIsReverseDraw) {
				mCircleCenterPointPositionList.add(mHeight
						- (mCircleRadius + i * mCircleRadius * 2 + i
								* mLinePadding));
			} else {
				mCircleCenterPointPositionList.add(mCircleRadius + i
						* mCircleRadius * 2 + i * mLinePadding);
			}
		}
		/**
		 * set listener
	...
	protected synchronized void onDraw(Canvas canvas) {
		...
		for (int i = 0; i < mCircleCenterPointPositionList.size() - 1; i++) {
			...
			final float preComplectedXPosition = mCircleCenterPointPositionList
					.get(i);
			// 后一个ComplectedXPosition
			final float afterComplectedXPosition = mCircleCenterPointPositionList
					.get(i + 1);
			...
			if (i < mComplectingPosition)// 判断在完成之前的所有点
			{
				...
				if (mIsReverseDraw) {
					canvas.drawRect(mLeftY, afterComplectedXPosition
							+ mCircleRadius - 10, mRightY,
							preComplectedXPosition - mCircleRadius + 10,
							mCompletedPaint);
				} else {
					canvas.drawRect(mLeftY, preComplectedXPosition
							+ mCircleRadius - 10, mRightY,
				...
				if (mIsReverseDraw) {
					mPath.moveTo(mCenterX, afterComplectedXPosition
							+ mCircleRadius);
					mPath.lineTo(mCenterX, preComplectedXPosition
							- mCircleRadius);
					canvas.drawPath(mPath, mUnCompletedPaint);
				} else {
					mPath.moveTo(mCenterX, preComplectedXPosition
							+ mCircleRadius);
					mPath.lineTo(mCenterX, afterComplectedXPosition
							- mCircleRadius);
					canvas.drawPath(mPath, mUnCompletedPaint);
				}
		...
		for (int i = 0; i < mCircleCenterPointPositionList.size(); i++) {
			...
			if (i < mComplectingPosition) {
				...
				canvas.drawCircle(mCenterX, currentComplectedXPosition,
						mCircleRadius * 1.1f, mCompletedPaint);
