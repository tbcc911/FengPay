package com.fzs.comn.widget.switcher;
...
public class UpDownSwitcher extends ViewSwitcher {
    ...
    public UpDownSwitcher(Context context) {
        this(context, null);
    }

    public UpDownSwitcher(Context context, AttributeSet attrs) {
        super(context, attrs);
    ...
    private void parseAttrs(Context context, AttributeSet attrs) {
        if (attrs != null) {
            TypedArray typedArray = context.getTheme().obtainStyledAttributes(attrs, R.styleable.UpDownSwitcher, 0, 0);
            isUpToDown = typedArray.getBoolean(R.styleable.UpDownSwitcher_up_to_down, true);
            animator_duration = typedArray.getInteger(R.styleable.UpDownSwitcher_animator_duration, 300);
            switch_duration = typedArray.getInteger(R.styleable.UpDownSwitcher_switch_duration, 3000);
            animator_translate_distance = typedArray.getDimension(R.styleable.UpDownSwitcher_animator_translate_distance,40);
        }else{
            isUpToDown = true;
    ...
    private static class AutoPlayTask implements Runnable {
        private final WeakReference<UpDownSwitcher> mSwitcher;

        private AutoPlayTask(UpDownSwitcher switcher) {
            mSwitcher = new WeakReference<>(switcher);
        }

        @Override
        public void run() {
            UpDownSwitcher switcher = mSwitcher.get();
            if (switcher != null) {
